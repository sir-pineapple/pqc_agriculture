...
void kyber_sensor_task(void *arg) {
    ESP_LOGI(TAG, "=== Soil Moisture + Kyber + WiFi ===");
    while (1) {
        int sock = -1;
        ...
        uint32_t seq = 0;
        while (1) {
            soil_moisture_data_t soil_reading;
            sensor_payload_t     payload;
            esp_err_t ret = soil_moisture_read(&soil_reading);
            if (ret != ESP_OK) {
                ESP_LOGW(TAG, "Soil moisture read failed, skipping");
                vTaskDelay(pdMS_TO_TICKS(2500));
                continue;
            }
            float voltage_mv = (soil_reading.raw / 4095.0f) * 3300.0f;
            payload.raw          = (int32_t)soil_reading.raw;
            payload.voltage_mv   = voltage_mv;
            payload.timestamp_ms = (uint32_t)(esp_timer_get_time() / 1000);
            payload.sensor_id    = 0x02;
            ESP_LOGI(TAG, "Raw: %d  Voltage: %.1f mV  Moisture: %.1f%%",
                     soil_reading.raw, voltage_mv, soil_reading.moisture_pct);
            uint8_t enc_buf[PACKET_IV_LEN + sizeof(payload) + PACKET_TAG_LEN];
            size_t  enc_len = 0;
            if (crypto_encrypt(&session, (uint8_t *)&payload, sizeof(payload),
